 ...
 * @param {String=} options.titleSize - Optional flag to set the title size on the report.
 * @param {String=} options.browserTitle - Optional flag to set a custom title on the HTML page.
 ...
 * @param {String=} options.testPaging - Optional flag to show paging in the report.
...
PostmanHTMLExtraReporter = function (newman, options, collectionRunOptions) {
    ...
    handlebars.registerHelper('paging', function () {
        return options.testPaging || false;
    });
    ...
    var obj = new ora(`Using ${chalk.green('htmlextra')} version ${chalk.green(version)}`)
    ...
    newman.on('done', function () {
        obj.succeed('HTMLEXTRA report created!')
        // eslint-disable-next-line max-len
        obj.succeed(`File export location: ${collectionRunOptions.reporter.htmlextra.export ? chalk.green(collectionRunOptions.reporter.htmlextra.export) : chalk.green('/newman')}`);
    });
    ...
    newman.on('beforeDone', function () {
        const items = {},
            ...
            assertions = _.transform(executions, function (result, currentExecution) {
                let stream,
                    reducedExecution;
                const executionId = currentExecution.cursor.ref;
